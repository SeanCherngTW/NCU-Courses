...
#         print(i)
    ...
    hidden_layer_5 = tf.get_default_graph().get_tensor_by_name("hidden_layer_5/Wx_plus_b_af:0")
    ...
    with tf.Session() as sess:
        ...
        # Since h1~h4 are frozen, the output of h4 would be the same
        # So we may cache the result to avoid repeatitive calculation
        h5_cache = sess.run(hidden_layer_5, feed_dict={X: X_train2_full})
...
#         writer = tf.summary.FileWriter(logdir, sess.graph)
        ...
        for i in range(epoch):
            ...
            for j in range(batch):
                batch_x = h5_cache[j * batch_size : (j + 1) * batch_size]
#                 batch_x = X_train2_full[j * batch_size : (j + 1) * batch_size]
                batch_y = y_train2_100[j * batch_size : (j + 1) * batch_size]
                sess.run(training_op, feed_dict={hidden_layer_5: batch_x, y: batch_y})
...
DNN(epoch=97, learning_rate=0.9, batch_size=50, early_stopping=1000)
